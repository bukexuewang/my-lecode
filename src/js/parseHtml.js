 ...
 */
class ParseHtml {
  static setAttribute(ele, attribute) {
    if (!attribute) {
      return;
    }
    if (attribute && ele) {
      for (let p in attribute) {
        if (attribute.hasOwnProperty(p)) {
          ele.setAttribute(p, attribute[p]);
        }
      }
    }
  }
  static render = (node) => {
    ...
    ParseHtml.setAttribute(ele, node.attribute);
    if (node.text) {
      ele.innerHTML = node.text;
    }
    if (node.children) {
      node.children.forEach((e) => ele.appendChild(ParseHtml.render(e)));
    }
    return ele;
  };

  static ast2Html(node) {
    const el = document.createElement('div');
    el.appendChild(ParseHtml.render(node));
